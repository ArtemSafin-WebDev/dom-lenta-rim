...
- Для всех новых элементов и секций сайта использовать префикс `dom-lenta` в классах и именовании.
...
- Не использовать фиксированную высоту (`height`), кроме иконок.
...
- Все растровые изображения сохранять и добавлять в проект только в формате `webp`.
- `SVG` не конвертировать в `webp` или другие растровые форматы.
- Для `img` в абсолютном позиционировании не использовать `inset: 0`; вместо этого явно задавать координаты и размеры: `top: 0; left: 0; width: 100%; height: 100%`.
...
- Не использовать чрезмерно большие размеры изображений: оптимизировать вес и габариты, сохраняя четкость на Retina-экранах.
